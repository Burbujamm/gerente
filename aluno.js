...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function salvarMetaNoLocalStorage() {
        localStorage.setItem('metaSemanal', aluno.metaSemanal);
    }
    ...
    function atualizarDados() {
        ...
        elementos.horasSemana.textContent = `${aluno.horasTreinadasSemana.toFixed(1)}h`;
    ...
    function carregarDados() {
        const treinosSalvos = localStorage.getItem('meusTreinos');
        const historicoSalvo = localStorage.getItem('historicoSemanal');
        const perfilFoto = localStorage.getItem('perfilFoto');
        const perfilNome = localStorage.getItem('perfilNome');
        const perfilEmail = localStorage.getItem('perfilEmail');
        const metaSalva = localStorage.getItem('metaSemanal');

        if (treinosSalvos) {
            aluno.meusTreinos = JSON.parse(treinosSalvos);
        }

        if (historicoSalvo) {
            aluno.historicoSemanal = JSON.parse(historicoSalvo);
        }

        if (perfilFoto) {
            elementos.imgPerfil.src = perfilFoto;
        }

        if (perfilNome) {
            elementos.perfilNome.value = perfilNome;
            aluno.nome = perfilNome;
            elementos.nomeAluno.textContent = perfilNome;
        }

        if (perfilEmail) {
            elementos.perfilEmail.value = perfilEmail;
            aluno.email = perfilEmail;
        }

        if (metaSalva) {
            aluno.metaSemanal = parseInt(metaSalva);
            elementos.valorMeta.textContent = `${aluno.metaSemanal}h`;
            elementos.metaSlider.noUiSlider.set(aluno.metaSemanal); // Atualiza o slider
        }
    ...
    document.getElementById('sair').addEventListener('click', (e) => {
        ...
        setTimeout(() => {
            window.location.href = 'login.html'; // Atualize para a URL correta
        }, 1500);
    });
    ...
    // **Função para Carregar Histórico de Treinos**
    function carregarHistoricoTreinos() {
        elementos.listaHistoricoTreinos.innerHTML = '';

        const totalHoras = aluno.historicoSemanal.reduce((a, b) => a + b, 0);
        if (totalHoras === 0) {
            elementos.listaHistoricoTreinos.innerHTML = '<p>Você ainda não realizou nenhum treino.</p>';
            return;
        }

        aluno.historicoSemanal.forEach((horas, dia) => {
            if (horas > 0) {
                const div = document.createElement('div');
                div.classList.add('historico-item');
                div.innerHTML = `
                    <p><strong>Dia:</strong> ${diaSemana(dia)}</p>
                    <p><strong>Horas Treinadas:</strong> ${horas.toFixed(1)}h</p>
                `;
                elementos.listaHistoricoTreinos.appendChild(div);
            }
        });
    }

    // **Função para Converter Índice do Dia para Nome**
    function diaSemana(indice) {
    ...
    // **Função para Carregar Exercícios Disponíveis no Modal de Montar Treino**
    function carregarExerciciosDisponiveisFinal() {
